        ...
        static char* prefix##_to_string(type v) {                               \
                ...
                BIT_FOREACH(i, v) {                                             \
                        ...
                        if (!strextend_with_separator(&s, "|", i->name))        \
                                return NULL;                                    \
                        v &= ~INDEX_TO_MASK(type, i)                            \
                }                                                               \
                                                                                \
                if (mask != 0 && strextendf_with_separator(&s, "|", format_str, v) < 0) \
                        return NULL;                                            \
                                                                                \
                return TAKE_PTR(s);                                             \
        }
...
int xstatx_full(int fd,
        ...
        if (!FLAGS_SET(sx.stx_attributes, mandatory_attributes)) {
                if (DEBUG_LOGGING) {
                        _cleanup_free_ char *attr_str = statx_attributes_to_string(mandatory_attributes & ~sx.stx_attributes);
