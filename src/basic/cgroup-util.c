...
static int cg_enumerate_items(int cgroupfs_fd, const char *path, const char *item, FILE **ret) {
        ...
        r = cg_get_path(cgroupfs_fd, path, item, &fs);
        if (r < 0)
                return r;
...
int cg_enumerate_processes(int cgroupfs_fd, const char *path, FILE **ret) {
        return cg_enumerate_items(cgroupfs_fd, path, "cgroup.procs", ret);
}
...
int cg_get_xattr(int cgroupfs_fd, const char *path, const char *name, char **ret) {
        _cleanup_free_ char *fs = NULL;
        int r;
        ...
        return getxattr_at_malloc(fs, name, ret);
}
...
static const char* skip_user_manager(const char *p) {
        ...
        if (!startswith(p, "user@") || !endswith_n(p, n, ".service"))
                return NULL;

        char buf[n - STRLEN("user@") - STRLEN(".service") + 1];

        memcpy(buf, p + 5, n - 5 - 8);
        buf[n - 5 - 8] = 0;

        /* No cg_unescape() here as with skip_session(). */
