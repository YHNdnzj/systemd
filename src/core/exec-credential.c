...
static int setup_credentials_plain_dir(
        ...
        if (renameat2(AT_FDCWD, workspace, AT_FDCWD, cred_dir, RENAME_EXCHANGE) < 0) {
                r = log_debug_errno(errno, "Failed to move credentials workspace into place: %m");
                goto fail_purge;
        }
...
static int setup_credentials_internal(
        ...
        if (ERRNO_IS_NEG_PRIVILEGE(mfd) && !dir_mounted) {
                ...
                return setup_credentials_plain_dir(...);
        }
        if (mfd < 0)
